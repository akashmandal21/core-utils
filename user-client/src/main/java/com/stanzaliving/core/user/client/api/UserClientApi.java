...
public class UserClientApi {
	...
	public ResponseDto<List<UserProfileDto>> getUserProfileListByUuidsListPost(List<String> userUuidList) {
		...
		try {
			...
			return restClient.invokeAPI(path, HttpMethod.POST, null, postBody, headerParams, accept, returnType);
		}
		catch (Exception ex) {
			log.error("Error occurred while fetching user details from UserId",ex);
			return null;
